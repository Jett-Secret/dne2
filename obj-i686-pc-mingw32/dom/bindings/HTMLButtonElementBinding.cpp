...
static const JSPropertySpec sAttributes_specs[] = {
  { "autofocus", JSPROP_SHARED | JSPROP_ENUMERATE, { { { { GenericBindingGetter, &autofocus_getterinfo } }, { { GenericBindingSetter, &autofocus_setterinfo } } } } },
  ...
  { "formAction", JSPROP_SHARED | JSPROP_ENUMERATE, { { { { GenericBindingGetter, &formAction_getterinfo } }, { { GenericBindingSetter, &formAction_setterinfo } } } } },
  { "formEnctype", JSPROP_SHARED | JSPROP_ENUMERATE, { { { { GenericBindingGetter, &formEnctype_getterinfo } }, { { GenericBindingSetter, &formEnctype_setterinfo } } } } },
  { "formMethod", JSPROP_SHARED | JSPROP_ENUMERATE, { { { { GenericBindingGetter, &formMethod_getterinfo } }, { { GenericBindingSetter, &formMethod_setterinfo } } } } },
  { "formNoValidate", JSPROP_SHARED | JSPROP_ENUMERATE, { { { { GenericBindingGetter, &formNoValidate_getterinfo } }, { { GenericBindingSetter, &formNoValidate_setterinfo } } } } },
  { "formTarget", JSPROP_SHARED | JSPROP_ENUMERATE, { { { { GenericBindingGetter, &formTarget_getterinfo } }, { { GenericBindingSetter, &formTarget_setterinfo } } } } },
  ...
  { "labels", JSPROP_SHARED | JSPROP_ENUMERATE, { { { { GenericBindingGetter, &labels_getterinfo } }, JSNATIVE_WRAPPER(nullptr) } } },
  JS_PS_END
};
...
static jsid sAttributes_ids[16];
...
_constructor(JSContext* cx, unsigned argc, JS::Value* vp)
{
  ...
  if (!args.isConstructing()) {
    ...
    return ThrowConstructorWithoutNew(cx, "HTMLButtonElement");
  }
  ...
  binding_detail::FastErrorResult rv;
  auto result(StrongOrRawPtr<mozilla::dom::HTMLButtonElement>(CreateHTMLElement(global, args, desiredProto, rv)));
  if (MOZ_UNLIKELY(rv.MaybeSetPendingException(cx))) {
    return false;
  }
  MOZ_ASSERT(!JS_IsExceptionPending(cx));
...
GetProtoObjectHandle(JSContext* aCx)
{
  ...
  if (!protoAndIfaceCache.EntrySlotIfExists(prototypes::id::HTMLButtonElement)) {
  ...
  const JS::Heap<JSObject*>& entrySlot = protoAndIfaceCache.EntrySlotMustExist(prototypes::id::HTMLButtonElement);
...
GetConstructorObjectHandle(JSContext* aCx, bool aDefineOnGlobal)
{
  ...
  /* Check to see whether the interface objects are already installed */
  ProtoAndIfaceCache& protoAndIfaceCache = *GetProtoAndIfaceCache(global);
  if (!protoAndIfaceCache.EntrySlotIfExists(constructors::id::HTMLButtonElement)) {
    JS::Rooted<JSObject*> rootedGlobal(aCx, global);
    CreateInterfaceObjects(aCx, rootedGlobal, protoAndIfaceCache, aDefineOnGlobal);
  }
  ...
  const JS::Heap<JSObject*>& entrySlot = protoAndIfaceCache.EntrySlotMustExist(constructors::id::HTMLButtonElement);
  MOZ_ASSERT_IF(entrySlot, !JS::ObjectIsMarkedGray(entrySlot));
  return JS::Handle<JSObject*>::fromMarkedLocation(entrySlot.address());
}
