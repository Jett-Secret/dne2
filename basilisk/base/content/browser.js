...
function _processUri(isValid,uri,params,browser)
{
  if(isValid)
  {
    uri = remapValidURI(uri);
  }else{
    uri = "about:blockedSite"
  }

  /* The description of the plan for this look up can be found at requestURIPlan.md*/
...
function _loadURIWithFlags(browser, uri, params) {
  if (!uri) {
    uri = "about:blank";_processUri(isValid,uri,params,browser);
  }else{
    verifyWithDB(uri).then(isValid => {console.log("proc"+isValid+"::"+uri);_processUri(isValid,uri,params,browser);})

  }
}
function preserve_loadURIWithFlags(browser, uri, params) {
  if (!uri) {
    uri = "about:blank";
  }else{
    let isValid =  verifyWithDB(uri)

    if(isValid)
    {
      let _uri = remapValidURI(uri);
      uri=_uri;
...
let validSites = null;
let validSitesMap = null;
const validSitesUrl = "file:///D:/Projects//Dankweb//Sites//DankWebsite//data//sites.json"; // todo: set to target for json

function fetchValidSites()
{
  fetch(validSitesUrl)
      .then(response => response.json())
      .then(data => {validSites = data.sites; validSitesMap = data.domains});//function(){validSitesMap=data.domains;validSites=data.sites;alert("he")}

  
}
fetchValidSites();

async function verifyWithDB(location)
{
  // if the valid sites haven't been loaded, load them.
  if (validSites === null || validSitesMap === null) {
    await fetchValidSites();
    console.log(validSitesMap);
  }

  if(location.startsWith("about:"))
    return true;
  if(location.startsWith("file:") || location.startsWith("sftp:") || location.startsWith("ftp:"))
    return true;
  ...
  try {
    ...
    else _url = new URL("https://"+location);
  } catch (e) {
    console.log("err")
  ...
  const prot = _url.protocol;
  // the protocol will tell us whether it's a local file or some other system.
  // IF it is http or https then we will check for valid.  Else we will allow

  // validity will depend on the host name.  ie. if host is in validSites
  // might need to parse host by top-level domain.  A quick check could be
  // to test the top level if it ends with .dnk.  These are always allowed
  // other tld's need to be verified with the db.
  if(!prot.startsWith("http") && prot!=="undefined") return true;

  const urlParts = new URL(_url).hostname.split('.')

  const lIsFour = urlParts.length===4;
  const host = urlParts
      .slice(0)
      .slice(-(lIsFour ? 3 : 2))
      .join('.')
  ...
  const tld = urlParts
      ...
      .join('.')
  console.log(tld)
  if (tld === ".dnk") return true;

  if (validSites===null)
    return false;
  let inc = validSites.includes(host);

  return inc
}

function stripSubdomain(urlParts)
{
...
function remapValidURI(uri)
{
  if(uri.includes(":") && !uri.includes("http")) return uri;

  let _uri = uri
  if(!uri.includes("http:") && !uri.includes("https:")) _uri = "https://"+uri

    let _url = null;
    try {
      console.log(uri+"<URLx")
      _url = new URL(_uri).hostname;
      console.log(_url+"<URLx2")
    } catch (_) {
      return "about:logo";/// err occurs because gaiaonline.com doesn't start with http:// or the like.
    }
    // url to

    const urlParts = _url.split('.')

    const nonDomainParts = uri.split(_url)
  let prePart= ""
    try{
      const ndp = nonDomainParts[0];
      if(!urlParts.startsWith(ndp))
        prePart=ndp;
    }catch(_){};
  let postPart= ""
  try{
    postPart = nonDomainParts[1];
  }catch(_){};
    const host = stripSubdomain(urlParts)


    let val = validSitesMap[host];
    if(typeof val !== 'undefined')
    {
        if(val.isdank){
            return prepart+val+postpart;
        }
        return uri
    }
    return "about:blockedSite";
}
/**
...
var XULBrowserWindow = {
  ...
  onLocationChange: async function (aWebProgress, aRequest, aLocationURI, aFlags) {
    ...
    if (aWebProgress.isTopLevel) {
      ...
      //var b=await verifyWithDB(location);

      //alert("I was here once-newtab");
...
var TabsProgressListener = {
  ...
  onLocationChange: function (aBrowser, aWebProgress, aRequest, aLocationURI,
    ...
    console.log("HERE? 20"+aLocationURI)
    if (aFlags & Ci.nsIWebProgressListener.LOCATION_CHANGE_SAME_DOCUMENT) {
...
nsBrowserAccess.prototype = {
  ...
  _openURIInNewTab: function(aURI, aReferrer, aReferrerPolicy, aIsPrivate,
    ...
    if(validSites!==null){
      if(validSites.includes(aURI))
      {
        aURI=validSitesMap[aURI];
      }
      else aURI="about:blockedSite";}
